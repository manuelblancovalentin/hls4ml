...
def test_vivado_writer_emits_trainable_configs_and_copies_headers(tmp_path):
    ...
    assert any(port['name'] == 'controller_raw_update_norm_sq' for port in writer._make_trainable_top_level_ports(model))
    ...
    assert 'controller_raw_update_norm_sq[1];' in writer._make_trainable_testbench_data(model, '', 'e')
    assert 'controller_raw_update_norm_sq[0] = 0;' in writer._make_trainable_bridge_defaults(model, '')
    ...
    assert writer._make_dense_weight_trace_header(model.graph['dense']).endswith('weight_0_0')
    assert writer._make_dense_bias_trace_header(model.graph['dense']).endswith('bias_0')
    assert 'std::system("mkdir -p tb_data/training/dense");' in writer._make_trainable_parameter_trace_setup(model, '')
    assert 'std::ofstream f_dense_weights("tb_data/training/dense/weights.dat");' in writer._make_trainable_parameter_trace_streams(model, '')
    assert 'write_trainable_metadata(f_dense_weights, "dense/weights", run_datetime);' in writer._make_trainable_parameter_trace_headers(model, '')
    assert 'for (unsigned i = 0; i < 1; i++)' in writer._make_trainable_parameter_trace_values(model, '')
    assert 'f_dense_weights << "," << (double)w2[i];' in writer._make_trainable_parameter_trace_values(model, '')
